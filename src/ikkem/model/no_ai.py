...
# This file is a dedicated A0 counterfactual based on the _357 model. It keeps
# the _357 extreme-CF input/provenance support and the original model/config
# stack intact, but forces the effective internal scenario to NO_AI so an
# inherited runner cannot accidentally add AI load.
AI_OPERATIONAL_SCENARIOS = {"NO_AI"}
NO_AI_MODEL_VARIANT = True
...
def National_energy_model(args):
    ...
    ai_operational_scenario = "NO_AI"
    no_ai_scenario = True
    print(
        "Dedicated No-AI model variant: forcing effective scenario NO_AI; "
        f"requested parser-compatible scenario was {requested_ai_operational_scenario!r}."
    )
    ...
    ai_s4_zone_runtime_transfer_active = False
    ...
    # The dedicated A0 model never consumes an AI interface.  Preserve the
    # requested flag in metadata, but make the effective flag false.
    use_external_ai_load = requested_use_external_ai_load and not no_ai_scenario
    ...
    if strict_cluster_ai_interface and not use_external_ai_load and not no_ai_scenario:
        raise ValueError(
    ...
    if no_ai_scenario:
        # A0 uses exactly the same non-AI demand construction as the other
        # scenarios, but deliberately bypasses every AI interface and pool.
        ...
        for pro in Province:
            ...
            if idx != -1:
                base_load = (
                    np.asarray(load_demand[:, idx], dtype=float) * load_reshapping
                )[:HOURS]
            else:
                base_load = np.zeros(HOURS, dtype=float)

            non_ai_base_load_mw[pro] = base_load
            LOAD_DEMAND[pro] = np.array(base_load, dtype=float, copy=True)
            fixed_ai_load[pro] = np.zeros(HOURS, dtype=float)
            flexible_ai_load[pro] = np.zeros(HOURS, dtype=float)
            ai_hosting_capacity_gw[pro] = 0.0
            ai_hosting_penalty_rmb_per_mwh[pro] = 0.0
            ai_flexible_load_gw[pro] = 0.0
            ai_hosting_lb_gw[pro] = 0.0
            ai_hosting_ub_gw[pro] = 0.0
        ...
        all_AI_load = 0.0
        hourly_AI_load = 0.0
        total_with_ai_energy_mwh = non_ai_base_energy_mwh
        ai_to_non_ai_base_ratio = 0.0
        ai_share_of_total_after_addition = 0.0
        flexible_share_of_ai = None

        # Disable all AI-only policy and cost switches in the effective A0 run.
        requested_include_ai_penalty_in_objective = bool(
            include_ai_penalty_in_objective
        )
        include_ai_penalty_in_objective = False
        compute_ai_penalty_diagnostic = False
        ai_migration_ctilde = 0.0
        c_mig_per_gw_year = 0.0
        ai_s4_zone_runtime_transfer = False
        ai_s4_zone_runtime_transfer_active = False
        use_network_tier_caps = False
        ai_od_network_policy_file = ""
        ai_use_destination_tier_fallback = False
        ...
        ai_interface_metadata = {
            "scenario": "NO_AI",
            "effective_ai_scenario": "NO_AI_BASELINE",
            "ai_operational_scenario": "NO_AI",
            "extreme_cf": dict(extreme_cf_metadata),
            "requested_ai_operational_scenario_ignored": (
                requested_ai_operational_scenario
            ),
            "dedicated_no_ai_model_variant": bool(NO_AI_MODEL_VARIANT),
            "ai_workload_deliberately_excluded": True,
            "interface_bypassed_for_no_ai": True,
            "use_external_ai_load": False,
            "external_ai_load_requested_but_ignored": bool(
                requested_use_external_ai_load
            ),
            "use_source_cluster_ai_interface": False,
            "source_cluster_interface_active": False,
            "strict_cluster_ai_interface": bool(strict_cluster_ai_interface),
            "year": int(args.test_years),
            "hours": int(HOURS),
            "h2_scenario": h2_scenario,
            "h2_aboveground_allowed": bool(h2_aboveground_allowed),
            "h2_underground_allowed": bool(h2_underground_allowed),
            "dual_pool_interface": {
                "dual_pool": False,
                "siting_pool_gw": 0.0,
                "runtime_pool_gw": 0.0,
                "note": "Not applicable: AI workload is excluded in A0.",
            },
            "scenario_definitions": {
                "NO_AI": (
                    "A0 system baseline: non-AI electricity demand only; all fixed, "
                    "siting-flexible and runtime-flexible AI workload is excluded."
                )
            },
            "ai_penalty_policy": {
                "requested_include_ai_penalty_in_objective": (
                    requested_include_ai_penalty_in_objective
                ),
                "include_ai_penalty_in_objective": False,
                "compute_ai_penalty_diagnostic": False,
                "interpretation": "AI-only penalties are not applicable in A0.",
                "recommended_main_result_cost": "reported_power_system_cost",
            },
            "ai_target_check": {
                "target_base_ratio": 0.0,
                "target_final_share": 0.0,
                "target_flexible_share": None,
                "skipped_external_ai_targets": True,
                "passed": True,
                "reason": "No-AI baseline intentionally excludes the AI interface.",
            },
            "energy_accounting_mwh": energy_accounting_mwh,
            "ai_energy_accounting": {
                "definition": (
                    "A0 contains the same scaled non-AI base demand and zero AI "
                    "electricity demand."
                ),
                **energy_accounting_mwh,
            },
            "note": (
                "Dedicated No-AI model. AI_LOAD_FILE, ai_ratio and AI_load_shift "
                "do not add AI demand in this variant."
            ),
        }
    elif use_external_ai_load:
    ...
    if no_ai_scenario:
        print("No-AI baseline summary:")
        print(f"  effective scenario: {ai_interface_metadata.get('effective_ai_scenario')}")
        print(f"  non-AI baseline electricity: {non_ai_base_energy_mwh / 1e9:.4f} PWh")
        print("  fixed AI electricity: 0.0000 PWh")
        print("  flexible AI electricity: 0.0000 PWh")
        print("  total AI electricity: 0.0000 PWh")
        print(f"  total electricity: {total_with_ai_energy_mwh / 1e9:.4f} PWh")
        print("  flexible AI pool: 0.0000 GW")
        print(
            "  power-balance RHS electricity (non-AI base only): "
            f"{np.sum(total_load_demand) / 1e9:.4f} PWh"
        )
    elif use_external_ai_load:
        print(f"Non-AI baseline electricity: {non_ai_base_energy_mwh / 1e9:.4f} PWh")
    ...
    if (not no_ai_scenario) and compute_ai_penalty_diagnostic and (
    ...
    if no_ai_scenario:
        ...
        print("  scenario: NO_AI")
        print("  effective_ai_scenario: NO_AI_BASELINE")
        print("  source-cluster interface active: False")
        print("  AI_OD_FLOW active: False")
        print("  AI_BATCH_CUM active: False")
        print("  flexible pool GW: 0.0")
    elif use_external_ai_load:
        print("AI scenario summary:")
    ...
    if no_ai_scenario:
        no_ai_unexpected_structures = {
            "AI_OD_FLOW": bool(AI_OD_FLOW),
            "AI_RT_OD_FLOW": bool(AI_RT_OD_FLOW),
            "AI_RT_ZONE_ROUTE": bool(AI_RT_ZONE_ROUTE),
            "AI_UNMET_SITING": bool(AI_UNMET_SITING),
            "AI_UNMET_RUNTIME": bool(AI_UNMET_RUNTIME),
            "AI_BATCH_CUM": bool(AI_BATCH_CUM),
        }
        if use_external_ai_load or use_source_cluster_ai_interface:
            raise RuntimeError(
                "NO_AI invariant failed: an external/source-cluster AI interface is active."
            )
        if any(no_ai_unexpected_structures.values()):
            raise RuntimeError(
                "NO_AI invariant failed: AI-only optimization structures are active: "
                f"{no_ai_unexpected_structures}"
            )
        if abs(float(hourly_AI_load)) > 1e-12 or abs(float(all_AI_load)) > 1e-9:
            raise RuntimeError(
                "NO_AI invariant failed: the aggregate AI workload is not zero."
            )
        for pro in Province:
            pro_key = str(pro)
            load_gap_mw = float(
                np.max(
                    np.abs(
                        np.asarray(LOAD_DEMAND[pro_key], dtype=float)
                        - np.asarray(non_ai_base_load_mw[pro_key], dtype=float)
                    )
                )
            )
            fixed_max_mw = float(
                np.max(np.abs(np.asarray(fixed_ai_load[pro_key], dtype=float)))
            )
            flexible_max_mw = float(
                np.max(np.abs(np.asarray(flexible_ai_load[pro_key], dtype=float)))
            )
            if max(load_gap_mw, fixed_max_mw, flexible_max_mw) > 1e-9:
                raise RuntimeError(
                    "NO_AI invariant failed before solve for province "
                    f"{pro_key}: load_gap_mw={load_gap_mw}, "
                    f"fixed_ai_max_mw={fixed_max_mw}, "
                    f"flexible_ai_max_mw={flexible_max_mw}."
                )
        ...
        if model.status == gp.GRB.INFEASIBLE:
            ...
            for c in model.getConstrs():
                if c.IISConstr:
                    print(f" - {c.constrName}")
        print(
            f"[SOLVER] status after DualReductions=0 re-solve: "
            f"{int(model.status)} "
            f"({_status_name_map.get(model.status, 'OTHER')}), "
            f"SolCount={model.SolCount}"
        )
    ...
    if has_usable_solution and model.SolCount > 0:
        ...
        ai_interface_metadata["ai_batch_run_result"] = {
            ...
            "note": (
                ...
                "NO_AI/S0/S1 arrays are saved as zeros for downstream compatibility."
            ),
        }
        ...
        ai_interface_metadata["ai_batch_cum_result"] = {
            ...
            "note": (
                "AI_BATCH_CUM variables are only created for S4-OD. "
                "NO_AI, S0 and S1 save zero arrays for downstream compatibility."
            ),
        }
        ...
        if no_ai_scenario:
            conservation_arrival_total_gwh = 0.0
            ...
            conservation_basis = (
                "Explicit A0 NO_AI baseline: no AI workload arrives, is allocated, "
                "or is executed."
            )
        elif use_external_ai_load:
            if ai_operational_scenario == "S0":
        ...
        no_ai_zero_tolerance = 1e-9
        no_ai_solution_power_tolerance_gw = max(
            no_ai_zero_tolerance,
            float(ai_power_audit_tol_gw),
        )
        no_ai_zero_audit_status = "NOT_APPLICABLE"
        no_ai_zero_audit = {
            "applicable": False,
            "scenario": ai_operational_scenario,
            "status": no_ai_zero_audit_status,
        }
        if no_ai_scenario:
            load_demand_vs_non_ai_max_abs_mw = max(
                (
                    float(
                        np.max(
                            np.abs(
                                np.asarray(LOAD_DEMAND[str(pro)], dtype=float)
                                - np.asarray(
                                    non_ai_base_load_mw[str(pro)], dtype=float
                                )
                            )
                        )
                    )
            ...
            fixed_ai_input_max_abs_mw = max(
                (
                    float(
                        np.max(
                            np.abs(
                                np.asarray(
                                    fixed_ai_load.get(str(pro), np.zeros(HOURS)),
                                    dtype=float,
                                )
                            )
                        )
            ...
            flexible_ai_input_max_abs_mw = max(
                (
                    float(
                        np.max(
                            np.abs(
                                np.asarray(
                                    flexible_ai_load.get(str(pro), np.zeros(HOURS)),
                                    dtype=float,
                                )
                            )
                        )
            ...
            planned_ai_load_max_abs_gw = max(
                (
                    float(
                        np.max(
                            np.abs(
                                np.asarray(
                                    results_AI_load.get(pro, np.zeros(1)),
                                    dtype=float,
                                )
                            )
                        )
            ...
            batch_run_max_abs_gw = max(
                (
                    float(
                        np.max(
                            np.abs(
                                np.asarray(
                                    results_AI_batch_run.get(pro, np.zeros(HOURS)),
                                    dtype=float,
                                )
                            )
                        )
            ...
            batch_cum_max_abs_gwh = max(
                (
                    float(
                        np.max(
                            np.abs(
                                np.asarray(
                                    results_AI_batch_cum.get(pro, np.zeros(HOURS)),
                                    dtype=float,
                                )
                            )
                        )
            ...
            ai_energy_fields_max_abs_mwh = max(
                abs(float(fixed_ai_energy_mwh)),
                abs(float(flexible_ai_energy_mwh)),
                abs(float(ai_total_energy_mwh)),
                abs(float(all_AI_load)),
            )
            total_energy_vs_non_ai_abs_gap_mwh = abs(
                float(total_with_ai_energy_mwh)
                - float(non_ai_base_energy_mwh)
            )
            ai_capacity_fields_max_abs_gw = max(
                (
                    abs(float(value))
                    for mapping in (
                        ai_hosting_capacity_gw,
            ...
            ai_source_cluster_state_empty = not any(
                (
                    bool(ai_source_cluster_arrival_mw),
                    bool(ai_source_cluster_profile),
                    bool(ai_source_cluster_mean_mw),
                    bool(ai_source_cluster_runtime_mean_mw),
                    bool(ai_source_cluster_runtime_profile),
                    bool(ai_source_cluster_siting_mean_mw),
                    bool(ai_source_cluster_zone),
                    bool(ai_source_cluster_type),
                    abs(float(ai_interface_siting_pool_gw)) > no_ai_zero_tolerance,
                    abs(float(ai_interface_runtime_pool_gw)) > no_ai_zero_tolerance,
                )
            )
            ai_structures_empty = not any(
                (
                    bool(AI_OD_FLOW),
                    bool(AI_RT_OD_FLOW),
                    bool(AI_RT_ZONE_ROUTE),
                    bool(AI_UNMET_SITING),
                    bool(AI_UNMET_RUNTIME),
                    bool(AI_BATCH_CUM),
                    bool(ai_s4_od_params),
                    bool(use_source_cluster_ai_interface),
                )
            )
            ai_cost_terms_empty = not any(
                (
                    bool(ai_hosting_preference_penalty),
                    bool(ai_batch_hosting_preference_penalty),
                    bool(migration_cost),
                    bool(unmet_penalty),
                )
            )
            no_ai_checks = {
                "non_ai_base_energy_is_positive": bool(
                    non_ai_base_energy_mwh > 0.0
                ),
                "load_demand_equals_non_ai_base": bool(
                    load_demand_vs_non_ai_max_abs_mw <= no_ai_zero_tolerance
                ),
                "fixed_ai_input_is_zero": bool(
                    fixed_ai_input_max_abs_mw <= no_ai_zero_tolerance
                ),
                "flexible_ai_input_is_zero": bool(
                    flexible_ai_input_max_abs_mw <= no_ai_zero_tolerance
                ),
                "planned_ai_load_is_zero": bool(
                    planned_ai_load_max_abs_gw
                    <= no_ai_solution_power_tolerance_gw
                ),
                "batch_run_is_zero": bool(
                    batch_run_max_abs_gw
                    <= no_ai_solution_power_tolerance_gw
                ),
                "batch_cumulative_is_zero": bool(
                    batch_cum_max_abs_gwh <= no_ai_zero_tolerance
                ),
                "ai_energy_fields_are_zero": bool(
                    ai_energy_fields_max_abs_mwh <= no_ai_zero_tolerance
                ),
                "total_energy_equals_non_ai_base": bool(
                    total_energy_vs_non_ai_abs_gap_mwh
                    <= no_ai_zero_tolerance
                ),
                "ai_capacity_fields_are_zero": bool(
                    ai_capacity_fields_max_abs_gw <= no_ai_zero_tolerance
                ),
                "source_cluster_state_is_empty": bool(
                    ai_source_cluster_state_empty
                ),
                "ai_only_structures_are_empty": bool(ai_structures_empty),
                "ai_only_cost_terms_are_empty": bool(ai_cost_terms_empty),
                "external_ai_interface_is_disabled": bool(
                    not use_external_ai_load
                    and not use_source_cluster_ai_interface
                ),
                "ai_penalty_is_disabled": bool(
                    not include_ai_penalty_in_objective
                    and not compute_ai_penalty_diagnostic
                ),
            }
            no_ai_zero_audit_passed = all(no_ai_checks.values())
            no_ai_zero_audit_status = (
                "PASS_STRICT"
                if no_ai_zero_audit_passed
            ...
            no_ai_zero_audit = {
                ...
                "effective_ai_scenario": "NO_AI_BASELINE",
                "status": no_ai_zero_audit_status,
                "passed": bool(no_ai_zero_audit_passed),
                "input_zero_tolerance": float(no_ai_zero_tolerance),
                "solution_power_zero_tolerance_gw": float(
                    no_ai_solution_power_tolerance_gw
                ),
                "checks": no_ai_checks,
                "max_abs_residuals": {
                    "load_demand_vs_non_ai_base_mw": float(
                        load_demand_vs_non_ai_max_abs_mw
                    ),
                    "fixed_ai_input_mw": float(fixed_ai_input_max_abs_mw),
                    "flexible_ai_input_mw": float(
                        flexible_ai_input_max_abs_mw
                    ),
                    "planned_ai_load_gw": float(planned_ai_load_max_abs_gw),
                    "batch_run_gw": float(batch_run_max_abs_gw),
                    "batch_cumulative_gwh": float(batch_cum_max_abs_gwh),
                    "ai_energy_fields_mwh": float(
                        ai_energy_fields_max_abs_mwh
                    ),
                    "total_energy_vs_non_ai_base_mwh": float(
                        total_energy_vs_non_ai_abs_gap_mwh
                    ),
                    "ai_capacity_fields_gw": float(
                        ai_capacity_fields_max_abs_gw
                    ),
                },
                "ai_only_structure_presence": {
                    "AI_OD_FLOW": bool(AI_OD_FLOW),
                    "AI_RT_OD_FLOW": bool(AI_RT_OD_FLOW),
                    "AI_RT_ZONE_ROUTE": bool(AI_RT_ZONE_ROUTE),
                    "AI_UNMET_SITING": bool(AI_UNMET_SITING),
                    "AI_UNMET_RUNTIME": bool(AI_UNMET_RUNTIME),
                    "AI_BATCH_CUM": bool(AI_BATCH_CUM),
                    "source_cluster_interface": bool(
                        use_source_cluster_ai_interface
                    ),
                },
                "interpretation": (
                    "PASS_STRICT certifies that the solved A0 result contains only "
                    "the non-AI base demand and no AI workload, routing, batch, or "
                    "AI-only objective term."
                ),
            }
            ai_interface_metadata["no_ai_zero_audit"] = no_ai_zero_audit
        ...
        if no_ai_scenario and no_ai_zero_audit_status == "FAIL_STRUCTURAL":
        ...
        if no_ai_scenario:
            destination_power_cap_audit_status = "NOT_APPLICABLE"
            destination_power_cap_audit_passed = None
            destination_power_cap_audit_strict_passed = None
            destination_power_cap_audit_not_failed = None
        else:
        ...
        solver_strict_for_main_analysis = bool(model.status == GRB.OPTIMAL)
        ...
        requires_manual_review = (
            ...
            or (
                ai_audit_usable_for_main_analysis
                and not solver_strict_for_main_analysis
            )
        )
        ...
        ai_execution_audit = {
            ...
            "ai_audit_usable_for_main_analysis": bool(
                ai_audit_usable_for_main_analysis
            ),
            "no_ai_baseline": bool(no_ai_scenario),
            "no_ai_zero_audit_status": no_ai_zero_audit_status,
            "no_ai_zero_audit": no_ai_zero_audit,
            ...
            "usable_for_main_analysis_rule": (
                "Automatic main-analysis use requires OPTIMAL solver status, an AI "
                "audit status of PASS_STRICT or WARN_NUMERICAL_RESIDUAL, and a "
                "complete required-result file contract. Other feasible solver "
                "statuses are retained for manual review only."
            ),
        ...
        if no_ai_scenario:
            with open(
                os.path.join(res_dir, "no_ai_audit.json"),
                "w",
                encoding="utf-8",
            ) as f:
                json.dump(no_ai_zero_audit, f, indent=2, ensure_ascii=False)
        ...
        audit_summary_row = {
            ...
            "no_ai_baseline": bool(no_ai_scenario),
            "no_ai_zero_audit_status": no_ai_zero_audit_status,
            "no_ai_zero_audit_passed": bool(
                no_ai_zero_audit_status == "PASS_STRICT"
            ),
            "effective_ai_scenario": ai_interface_metadata.get(
                "effective_ai_scenario"
            ),
        ...
        if no_ai_scenario:
            required_result_files.extend(
                [
                    os.path.join(res_dir, "no_ai_audit.json"),
                    os.path.join(res_dir_pkl, "non_ai_base_load_mw.pkl"),
                    os.path.join(res_dir_pkl, "fixed_ai_load.pkl"),
                    os.path.join(res_dir_pkl, "flexible_ai_load.pkl"),
                ]
            )
        ...
        ai_audit_hard_failed = bool(hard_failed)
        usable_for_main_analysis = bool(
            usable_for_main_analysis and required_result_files_complete
        )
        requires_manual_review = bool(
            requires_manual_review or result_contract_failed
        )
        hard_failed = bool(ai_audit_hard_failed or result_contract_failed)

        # Keep every status surface consistent with the solver and output-file
        # policy after the required-result contract is known.
        ai_execution_audit["required_result_files_complete"] = (
            required_result_files_complete
        )
        ai_execution_audit["required_result_files_missing"] = (
            required_result_files_missing
        )
        ai_execution_audit["result_contract_failed"] = bool(
            result_contract_failed
        )
        ai_execution_audit["ai_audit_hard_failed"] = bool(
            ai_audit_hard_failed
        )
        ...
        ai_interface_metadata["ai_execution_audit"] = ai_execution_audit
        audit_summary_row["required_result_files_complete"] = (
            required_result_files_complete
        )
        audit_summary_row["result_contract_failed"] = bool(
            result_contract_failed
        )
        audit_summary_row["usable_for_main_analysis"] = bool(
            usable_for_main_analysis
        )
        audit_summary_row["requires_manual_review"] = bool(
            requires_manual_review
        )
        audit_summary_row["hard_failed"] = bool(hard_failed)
        with open(
            os.path.join(res_dir, "ai_execution_audit.json"),
            "w",
            encoding="utf-8",
        ...
        run_status = {
            ...
            "solver_acceptable": bool(
                has_usable_solution and model.SolCount > 0
            ),
            "solver_has_usable_solution": bool(
                has_usable_solution and model.SolCount > 0
            ),
            "solver_strict_for_main_analysis": bool(
                solver_strict_for_main_analysis
            ...
            "result_contract_failed": bool(result_contract_failed),
            "overall_ai_audit_status": overall_ai_audit_status,
            "no_ai_baseline": bool(no_ai_scenario),
            "no_ai_zero_audit_status": no_ai_zero_audit_status,
            "no_ai_zero_audit_passed": bool(
            ...
            "no_ai_audit_written": os.path.exists(
                os.path.join(res_dir, "no_ai_audit.json")
            ),
            "ai_penalty_in_objective": bool(include_ai_penalty_in_objective),
            "include_ai_penalty_in_objective": bool(include_ai_penalty_in_objective),
            ...
            "slurm_should_fail_on_structural_ai_audit": bool(
                (no_ai_scenario and hard_failed)
                or (
            ...
            "no_ai_structural_failure_always_raises": bool(no_ai_scenario),
            ...
            "note": (
                ...
                "RAISE_ON_STRUCTURAL_AI_AUDIT_FAIL for external-AI scenarios. "
                "The dedicated NO_AI baseline always raises on structural failure."
            ),
            "usable_for_main_analysis_rule": (
                "Automatic main-analysis use requires OPTIMAL solver status, an AI "
                "audit status of PASS_STRICT or WARN_NUMERICAL_RESIDUAL, and a "
        ...
        if (no_ai_scenario and hard_failed) or (
            strict_postsolve_ai_audit and use_external_ai_load and hard_failed
        ):
            msg = (
                "AI audit structural failure after postprocess completed: "
                f"overall_ai_audit_status={overall_ai_audit_status}; "
                f"no_ai_zero_audit_status={no_ai_zero_audit_status}; "
                f"result_contract_failed={result_contract_failed}; "
                f"required_result_files_missing={required_result_files_missing}; "
                f"ai_energy_audit_status={ai_energy_audit_status}; "
                f"cluster_od_allocation_audit_status={cluster_od_allocation_audit_status}; "
                f"destination_power_cap_audit_status={destination_power_cap_audit_status}; "
                f"s4_od_no_advance_audit_status="
            ...
            if no_ai_scenario or raise_on_structural_ai_audit_fail:
        ...
        fail_status = {
            ...
            "overall_ai_audit_status": "NOT_RUN_NO_USABLE_SOLUTION",
        ...
        if no_ai_scenario:
            raise RuntimeError(
                "NO_AI solve produced no usable feasible solution; "
                "see run_status.json and solver log."
            )
        return {}
