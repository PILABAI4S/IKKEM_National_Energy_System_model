...
# Dedicated A3 counterfactual: long-term AI siting is fixed to the S0
# source-cluster origin reconstruction, while the runtime subset retains the
# S4 deadline-constrained execution and adjacent-zone routing machinery.
RT_ONLY_MODEL_VARIANT = True
RT_ONLY_COUNTERFACTUAL = "A3_RUNTIME_ONLY"
RT_ONLY_PLANNING_ANCHOR = "S0_CLUSTER_FIXED"
RT_ONLY_EFFECTIVE_SCENARIO = "RT_ONLY_FIXED_ORIGIN_ZONE_RUNTIME_TRANSFER"
RT_ONLY_RUNTIME_PROFILE_MODE = "MATCH_A1_SMOOTHED_CLUSTER_PROFILE"
...
def _build_s4_od_parameters(
    ...
    zone_neighbors=None,
    rt_only_fixed_origin_mode=False,
):
    ...
    In rt_only_fixed_origin_mode, planning-policy reachability and hosting filters
    are bypassed and only positive S0 origin-weight destinations are retained.
    """
    ...
    if unconstrained_arcs and not rt_only_fixed_origin_mode:
    ...
    for gid in source_clusters:
        ...
        for dest in destination_provinces:
            ...
            if rt_only_fixed_origin_mode:
                if float(origin_weight_by_cluster[gid].get(dest, 0.0)) <= 0.0:
                    continue
                tier = 1
                min_tier = 1
                share_cap = 1.0
            else:
                # Allow same-zone OR adjacent-zone destinations (when
                # zone_neighbors is provided).
                if zone_neighbors is not None:
                    if (
                        dest_zone != zone
                        and dest_zone not in zone_neighbors.get(zone, {zone})
                    ):
                        continue
                elif dest_zone != zone:
                    continue
                if use_network_tier_caps:
                    if (gid, dest) in od_network_tier:
                        ...
                        tier = int(
                            destination_network_tier.get(dest, default_missing_tier)
                        )
                    else:
                        tier = int(default_missing_tier)
                    min_tier = _required_min_tier_for_cluster(
                        cluster_type.get(gid, "")
                    )
                    share_cap = float(
                        od_share_cap.get(
                            (gid, dest), tier_default_share_cap.get(tier, 0.0)
                        )
                    )
                else:
                    tier = 1
                ...
                if host_cap_scenario != "legacy_tier":
                    share_cap = float(
                        od_class_share_cap.get(
                            dest,
                            AI_SCENARIO_OD_CAPS[host_cap_scenario].get(
                                host_class, 0.0
                            ),
                        )
                    )
                if share_cap <= 0:
                    continue
            # Track cross-zone arcs for migration cost and audit. A valid
            # RT-only origin reconstruction is checked to be same-zone later.
    ...
    # unchanged. The effective runtime profile is selected below: dedicated A3
    # reuses A1's cluster profile, while legacy callers may use the supplied
    # runtime profile.
    ...
    for gid in source_clusters:
        ...
        if not np.isfinite(rt_gw):
            raise ValueError(
                f"S4-OD cluster {gid} runtime mean contains NaN or Inf."
            )
    ...
    # A3 is intended to differ from A1 only through runtime execution
    # flexibility. Therefore the effective runtime arrival profile must use
    # the same smoothed cluster profile that constructs A1. The independently
    # supplied raw runtime profile is still validated and its difference from
    # A1 is retained as provenance, but it is not used by the main RT-only
    # counterfactual.
    runtime_cluster_profile = {}
    runtime_profile_input_available_by_cluster = {}
    runtime_profile_input_max_abs_diff_by_cluster = {}
    runtime_profile_effective_max_abs_diff_by_cluster = {}
    for gid in source_clusters:
        normalized_input_runtime_profile = None
        if source_cluster_runtime_profile and gid in source_cluster_runtime_profile:
            ...
            if not np.all(np.isfinite(rt_profile)):
                raise ValueError(
                    f"S4-OD cluster {gid} runtime profile contains NaN or Inf."
                )
            ...
            normalized_input_runtime_profile = rt_profile / rt_mean
            runtime_profile_input_available_by_cluster[gid] = True
            runtime_profile_input_max_abs_diff_by_cluster[gid] = float(
                np.max(
                    np.abs(
                        normalized_input_runtime_profile - cluster_profile[gid]
                    )
                )
            )
        else:
            runtime_profile_input_available_by_cluster[gid] = False
            runtime_profile_input_max_abs_diff_by_cluster[gid] = 0.0

        if rt_only_fixed_origin_mode:
            runtime_cluster_profile[gid] = np.array(
                cluster_profile[gid], dtype=float, copy=True
            )
        elif normalized_input_runtime_profile is not None:
            runtime_cluster_profile[gid] = normalized_input_runtime_profile
        else:
            runtime_cluster_profile[gid] = cluster_profile[gid]
        runtime_profile_effective_max_abs_diff_by_cluster[gid] = float(
            np.max(
                np.abs(runtime_cluster_profile[gid] - cluster_profile[gid])
            )
        )
    runtime_profile_mode = (
        RT_ONLY_RUNTIME_PROFILE_MODE
        if rt_only_fixed_origin_mode
        else "INPUT_RUNTIME_PROFILE_WHEN_AVAILABLE"
    )
    runtime_profile_input_max_abs_diff = float(
        max(runtime_profile_input_max_abs_diff_by_cluster.values(), default=0.0)
    )
    runtime_profile_effective_max_abs_diff = float(
        max(runtime_profile_effective_max_abs_diff_by_cluster.values(), default=0.0)
    )
    if rt_only_fixed_origin_mode and runtime_profile_effective_max_abs_diff > 1e-12:
        raise ValueError(
            "RT-only runtime profile contract failed: the effective runtime "
            "profile must exactly match the A1 smoothed cluster profile."
        )
    ...
    return {
        ...
        "runtime_profile_input_max_abs_diff": runtime_profile_input_max_abs_diff,
        "runtime_profile_effective_max_abs_diff_by_cluster": (
            runtime_profile_effective_max_abs_diff_by_cluster
        ),
        "runtime_profile_effective_max_abs_diff": (
            runtime_profile_effective_max_abs_diff
        ),
        ...
        "audit": {
            ...
            "rt_only_fixed_origin_mode": bool(rt_only_fixed_origin_mode),
            "runtime_profile_mode": runtime_profile_mode,
            "runtime_profile_input_available_by_cluster": (
                runtime_profile_input_available_by_cluster
            ),
            ...
            "runtime_profile_input_max_abs_diff": (
                runtime_profile_input_max_abs_diff
            ),
            "runtime_profile_effective_max_abs_diff_by_cluster": (
                runtime_profile_effective_max_abs_diff_by_cluster
            ...
            "runtime_profile_contract_note": (
                "A3 uses the A1 smoothed cluster profile for both the siting-only "
                "and runtime subsets. Any independently supplied raw runtime "
                "profile is retained only for provenance and sensitivity analysis."
            ),
...
def add_rt_only_fixed_origin_constraints(
    model,
    AI_OD_FLOW,
    PRO_AI_LOAD,
    ai_cluster_params,
    ...
    """Link the immutable S0-origin siting anchor into the shared S4 model.

    The variables are fixed through equal lower/upper bounds when they are
    created. These equalities are deliberately retained as named structural
    checks in the solver model. No destination energy quota or hosting-share
    planning constraint is added: those policies govern long-term relocation,
    which is disabled in the A3 Runtime-only counterfactual.
    """
    source_clusters = ai_cluster_params["source_clusters"]
    destination_provinces = ai_cluster_params["destination_provinces"]
    allowed_destinations_by_cluster = ai_cluster_params[
    ...
    clusters_by_destination = ai_cluster_params["clusters_by_destination"]
    cluster_mean_gw = ai_cluster_params["cluster_mean_gw"]
    fixed_destination_siting_gw = ai_cluster_params[
        "rt_only_fixed_destination_siting_gw"
    ]
    status = {
        "enabled": True,
        "planning_enabled": False,
        "planning_anchor": RT_ONLY_PLANNING_ANCHOR,
        "source_balance_constraints": 0,
        "destination_plan_link_constraints": 0,
        "destination_energy_quota_constraints": 0,
        "destination_hosting_share_constraints": 0,
        "fixed_origin_arc_count": int(
            sum(len(v) for v in allowed_destinations_by_cluster.values())
        ),
    }
    ...
    for gid in source_clusters:
        model.addConstr(
            ...
            == float(cluster_mean_gw[gid]),
            name=f"{prefix}_fixed_origin_source_balance_{gid}",
        )
        status["source_balance_constraints"] += 1
    ...
    for dest in destination_provinces:
        planned_dest = gp.quicksum(
            AI_OD_FLOW[gid][dest]
            for gid in clusters_by_destination[dest]
            if gid in AI_OD_FLOW and dest in AI_OD_FLOW[gid]
        )
        model.addConstr(
            PRO_AI_LOAD[dest] == planned_dest,
            name=f"{prefix}_fixed_origin_destination_link_{dest}",
        )
        model.addConstr(
            PRO_AI_LOAD[dest] == float(fixed_destination_siting_gw[dest]),
            name=f"{prefix}_fixed_origin_destination_target_{dest}",
        )
        status["destination_plan_link_constraints"] += 2
...
def configure_rt_only_fixed_origin_parameters(
    ai_cluster_params,
    provinces,
    province_zone,
    tolerance=1e-8,
):
    """Replace free planning arcs with immutable S0-origin siting targets.

    Runtime targets use the same origin weights as siting, so the long-term
    placement of both the siting-only and runtime pools is fixed. Runtime may
    subsequently change execution zone and execution hour through the existing
    S4 zone-route and deadline constraints.
    """
    provinces = [str(p) for p in provinces]
    province_set = set(provinces)
    source_clusters = list(ai_cluster_params.get("source_clusters", []))
    if not source_clusters:
        raise ValueError("RT-only requires non-empty source-cluster parameters.")

    fixed_siting_target_gw = {}
    fixed_runtime_target_gw = {}
    fixed_destination_siting_gw = {p: 0.0 for p in provinces}
    fixed_destination_runtime_gw = {p: 0.0 for p in provinces}
    allowed_destinations_by_cluster = {}
    clusters_by_destination = {p: [] for p in provinces}
    fixed_arc_count = 0

    for gid in source_clusters:
        weights = ai_cluster_params["origin_weight_by_cluster"].get(gid, {})
        if not weights:
            raise ValueError(f"RT-only cluster {gid} has no S0 origin weights.")
        weight_sum = float(sum(float(v) for v in weights.values()))
        if abs(weight_sum - 1.0) > tolerance:
            raise ValueError(
                f"RT-only cluster {gid} origin weights sum to {weight_sum}, "
                f"expected 1 within {tolerance}."
            )
        cluster_mean_gw = float(ai_cluster_params["cluster_mean_gw"][gid])
        runtime_mean_gw = float(
            ai_cluster_params["runtime_cluster_mean_gw"].get(gid, 0.0)
        )
        if runtime_mean_gw < -tolerance or runtime_mean_gw > cluster_mean_gw + tolerance:
            raise ValueError(
                f"RT-only cluster {gid} runtime mean {runtime_mean_gw} GW is not "
                f"a subset of siting mean {cluster_mean_gw} GW."
            )
        fixed_siting_target_gw[gid] = {}
        fixed_runtime_target_gw[gid] = {}
        fixed_origins = []
        for origin, raw_weight in sorted(weights.items()):
            origin = str(origin)
            weight = float(raw_weight)
            if weight < -tolerance:
                raise ValueError(
                    f"RT-only cluster {gid} has negative origin weight for {origin}: "
                    f"{weight}."
                )
            if weight <= 0.0:
                continue
            if origin not in province_set:
                raise ValueError(
                    f"RT-only cluster {gid} origin {origin} is outside the model "
                    "province set."
                )
            cluster_zone = str(ai_cluster_params["cluster_zone"].get(gid, ""))
            origin_zone = str(province_zone.get(origin, ""))
            if cluster_zone != origin_zone:
                raise ValueError(
                    f"RT-only S0-origin reconstruction mismatch for cluster {gid}: "
                    f"origin {origin} is in zone {origin_zone}, cluster zone is "
                    f"{cluster_zone}."
                )
            siting_target = cluster_mean_gw * weight
            runtime_target = runtime_mean_gw * weight
            fixed_siting_target_gw[gid][origin] = float(siting_target)
            fixed_runtime_target_gw[gid][origin] = float(runtime_target)
            fixed_destination_siting_gw[origin] += float(siting_target)
            fixed_destination_runtime_gw[origin] += float(runtime_target)
            fixed_origins.append(origin)
            clusters_by_destination[origin].append(gid)
            fixed_arc_count += 1
        if not fixed_origins:
            raise ValueError(f"RT-only cluster {gid} has no positive S0 origin weights.")
        allowed_destinations_by_cluster[gid] = fixed_origins

    siting_target_total_gw = float(
        sum(sum(v.values()) for v in fixed_siting_target_gw.values())
    )
    runtime_target_total_gw = float(
        sum(sum(v.values()) for v in fixed_runtime_target_gw.values())
    )
    expected_siting_total_gw = float(ai_cluster_params.get("siting_pool_gw", 0.0))
    expected_runtime_total_gw = float(ai_cluster_params.get("runtime_pool_gw", 0.0))
    if abs(siting_target_total_gw - expected_siting_total_gw) > tolerance:
        raise ValueError(
            "RT-only fixed-origin siting target does not conserve the siting pool: "
            f"target={siting_target_total_gw}, expected={expected_siting_total_gw}."
        )
    if abs(runtime_target_total_gw - expected_runtime_total_gw) > tolerance:
        raise ValueError(
            "RT-only fixed-origin runtime target does not conserve the runtime pool: "
            f"target={runtime_target_total_gw}, expected={expected_runtime_total_gw}."
        )

    ai_cluster_params["allowed_destinations_by_cluster"] = (
        allowed_destinations_by_cluster
    )
    ai_cluster_params["clusters_by_destination"] = clusters_by_destination
    ai_cluster_params["od_share_cap"] = {
        (gid, dest): 1.0
        for gid, destinations in allowed_destinations_by_cluster.items()
        for dest in destinations
    }
    ai_cluster_params["od_network_tier"] = {
        (gid, dest): 1
        for gid, destinations in allowed_destinations_by_cluster.items()
        for dest in destinations
    }
    ai_cluster_params["od_network_score"] = {}
    ai_cluster_params["cross_zone_arcs"] = set()
    ai_cluster_params["rt_only_fixed_siting_target_gw"] = fixed_siting_target_gw
    ai_cluster_params["rt_only_fixed_runtime_target_gw"] = fixed_runtime_target_gw
    ai_cluster_params["rt_only_fixed_destination_siting_gw"] = (
        fixed_destination_siting_gw
    )
    ai_cluster_params["rt_only_fixed_destination_runtime_gw"] = (
        fixed_destination_runtime_gw
    )
    ai_cluster_params["rt_only_fixed_origin_audit"] = {
        "counterfactual": RT_ONLY_COUNTERFACTUAL,
        "planning_enabled": False,
        "runtime_enabled": True,
        "planning_anchor": RT_ONLY_PLANNING_ANCHOR,
        "fixed_origin_arc_count": int(fixed_arc_count),
        "non_origin_planning_arc_count": 0,
        "siting_target_total_gw": siting_target_total_gw,
        "runtime_target_total_gw": runtime_target_total_gw,
        "siting_pool_abs_gap_gw": float(
            abs(siting_target_total_gw - expected_siting_total_gw)
        ),
        "runtime_pool_abs_gap_gw": float(
            abs(runtime_target_total_gw - expected_runtime_total_gw)
        ),
        "planning_destination_energy_caps_binding": False,
        "planning_hosting_share_caps_binding": False,
        "runtime_destination_power_caps_binding": True,
    }
    ai_cluster_params["audit"]["rt_only_fixed_origin"] = dict(
        ai_cluster_params["rt_only_fixed_origin_audit"]
    )
    ai_cluster_params["audit"]["allowed_od_arc_count"] = int(fixed_arc_count)
    ai_cluster_params["audit"]["cross_zone_arc_count"] = 0
    ai_cluster_params["audit"]["cross_zone_arc_share"] = 0.0
    ai_cluster_params["audit"]["max_allowed_destinations_per_cluster"] = int(
        max(len(v) for v in allowed_destinations_by_cluster.values())
    )
    ai_cluster_params["audit"]["zero_incoming_destination_count"] = int(
        sum(1 for v in clusters_by_destination.values() if len(v) == 0)
    )
    return ai_cluster_params
...
def audit_rt_only_zero_shift_a1_parity(
    ai_cluster_params,
    s0_cluster_fixed_flexible_load_mw,
    provinces,
    hours,
    ...
    """Prove that A3 with zero runtime shifting reproduces A1 hourly load."""
    provinces = [str(p) for p in provinces]
    if tolerance_mw < 0.0:
        raise ValueError("RT-only zero-shift parity tolerance must be non-negative.")

    zero_shift_load_mw = {
        pro: np.zeros(hours, dtype=float) for pro in provinces
    }
    by_cluster = {}
    max_cluster_gap_mw = 0.0

    for gid in ai_cluster_params.get("source_clusters", []):
        siting_profile = np.asarray(
            ai_cluster_params["cluster_profile"][gid], dtype=float
        )[:hours]
        runtime_profile = np.asarray(
            ai_cluster_params["runtime_cluster_profile"][gid], dtype=float
        )[:hours]
        if len(siting_profile) != hours or len(runtime_profile) != hours:
            raise ValueError(
                f"RT-only zero-shift parity profile length mismatch for {gid}."
            )
        if not (
            np.all(np.isfinite(siting_profile))
            and np.all(np.isfinite(runtime_profile))
        ):
            raise ValueError(
                f"RT-only zero-shift parity found NaN or Inf for {gid}."
            )

        siting_mean_gw = float(ai_cluster_params["cluster_mean_gw"][gid])
        runtime_mean_gw = float(
            ai_cluster_params["runtime_cluster_mean_gw"][gid]
        )
        a1_cluster_mw = siting_mean_gw * 1000.0 * siting_profile
        a3_zero_shift_cluster_mw = (
            (siting_mean_gw - runtime_mean_gw)
            * 1000.0
            * siting_profile
            + runtime_mean_gw * 1000.0 * runtime_profile
        )
        cluster_diff_mw = a3_zero_shift_cluster_mw - a1_cluster_mw
        cluster_max_gap_mw = float(np.max(np.abs(cluster_diff_mw)))
        max_cluster_gap_mw = max(max_cluster_gap_mw, cluster_max_gap_mw)
        by_cluster[str(gid)] = {
            "max_abs_gap_mw": cluster_max_gap_mw,
            "energy_gap_mwh": float(np.sum(cluster_diff_mw)),
            "siting_mean_gw": siting_mean_gw,
            "runtime_mean_gw": runtime_mean_gw,
        }

        fixed_siting_targets = ai_cluster_params[
            "rt_only_fixed_siting_target_gw"
        ][gid]
        fixed_runtime_targets = ai_cluster_params[
            "rt_only_fixed_runtime_target_gw"
        ][gid]
        for origin, siting_target_gw in fixed_siting_targets.items():
            origin = str(origin)
            if origin not in zero_shift_load_mw:
                raise ValueError(
                    f"RT-only zero-shift parity found unknown origin {origin}."
                )
            runtime_target_gw = float(fixed_runtime_targets[origin])
            zero_shift_load_mw[origin] += (
                (float(siting_target_gw) - runtime_target_gw)
                * 1000.0
                * siting_profile
                + runtime_target_gw * 1000.0 * runtime_profile
            )

    by_province = {}
    max_province_gap_mw = 0.0
    total_energy_gap_mwh = 0.0
    national_a1_mw = np.zeros(hours, dtype=float)
    national_a3_zero_shift_mw = np.zeros(hours, dtype=float)
    for pro in provinces:
        a1_load_mw = np.asarray(
            s0_cluster_fixed_flexible_load_mw.get(
                pro, np.zeros(hours, dtype=float)
            ),
            dtype=float,
        )[:hours]
        a3_zero_shift_mw = zero_shift_load_mw[pro]
        if len(a1_load_mw) != hours:
            raise ValueError(
                f"RT-only zero-shift parity A1 load length mismatch for {pro}."
            )
        diff_mw = a3_zero_shift_mw - a1_load_mw
        province_max_gap_mw = float(np.max(np.abs(diff_mw)))
        province_energy_gap_mwh = float(np.sum(diff_mw))
        max_province_gap_mw = max(max_province_gap_mw, province_max_gap_mw)
        total_energy_gap_mwh += province_energy_gap_mwh
        national_a1_mw += a1_load_mw
        national_a3_zero_shift_mw += a3_zero_shift_mw
        by_province[pro] = {
            "max_abs_gap_mw": province_max_gap_mw,
            "energy_gap_mwh": province_energy_gap_mwh,
        }

    national_diff_mw = national_a3_zero_shift_mw - national_a1_mw
    national_hourly_max_gap_mw = float(np.max(np.abs(national_diff_mw)))
    max_abs_gap_mw = max(
        max_cluster_gap_mw,
        max_province_gap_mw,
        national_hourly_max_gap_mw,
    )
    status = "PASS_STRICT" if max_abs_gap_mw <= tolerance_mw else "FAIL_STRUCTURAL"
    audit = {
        "status": status,
        "counterfactual": RT_ONLY_COUNTERFACTUAL,
        "profile_mode": ai_cluster_params.get("runtime_profile_mode"),
        "expected_profile_mode": RT_ONLY_RUNTIME_PROFILE_MODE,
        "tolerance_mw": float(tolerance_mw),
        "max_abs_gap_mw": float(max_abs_gap_mw),
        "max_cluster_gap_mw": float(max_cluster_gap_mw),
        "max_province_gap_mw": float(max_province_gap_mw),
        "national_hourly_max_abs_gap_mw": float(
            national_hourly_max_gap_mw
        ),
        "total_energy_gap_mwh": float(total_energy_gap_mwh),
        "by_cluster": by_cluster,
        "by_province": by_province,
        "identity": "(S-R)*p_A1 + R*p_A1 == S*p_A1",
        "interpretation": (
            "PASS_STRICT proves that disabling runtime time/zone shifting in A3 "
            "recovers the A1 fixed-origin flexible-AI load hour by hour."
        ),
    }
    if status != "PASS_STRICT":
        raise ValueError(
            "RT-only zero-shift A1 parity failed: "
            f"max_abs_gap_mw={max_abs_gap_mw}, tolerance_mw={tolerance_mw}."
        )
    return audit


def audit_rt_only_a1_reference_host_cap(
    fixed_ai_load,
    s0_cluster_fixed_flexible_load_mw,
    destination_power_cap_gw,
    provinces,
    ...
    """Check that the reused A1 hourly AI trajectory fits A3 physical caps."""
    provinces = [str(p) for p in provinces]
    by_province = {}
    max_violation_gw = 0.0
    violating_provinces = []

    for pro in provinces:
        if pro not in destination_power_cap_gw:
            raise ValueError(
                f"RT-only A1 host-cap audit is missing a cap for {pro}."
            )
        fixed_mw = np.asarray(
            fixed_ai_load.get(pro, np.zeros(hours, dtype=float)), dtype=float
        )[:hours]
        ...
        if len(fixed_mw) != hours or len(flexible_mw) != hours:
            raise ValueError(
                f"RT-only A1 host-cap audit load length mismatch for {pro}."
            )
        if not (
            np.all(np.isfinite(fixed_mw))
            and np.all(np.isfinite(flexible_mw))
        ):
            raise ValueError(
                f"RT-only A1 host-cap audit found NaN or Inf for {pro}."
            )
        a1_total_ai_gw = (fixed_mw + flexible_mw) / 1000.0
        cap_gw = float(destination_power_cap_gw[pro])
        if not np.isfinite(cap_gw):
            raise ValueError(
                f"RT-only A1 host-cap audit cap contains NaN or Inf for {pro}."
            )
        peak_hour = int(np.argmax(a1_total_ai_gw))
        peak_gw = float(a1_total_ai_gw[peak_hour])
        violation_gw = max(0.0, peak_gw - cap_gw)
        max_violation_gw = max(max_violation_gw, violation_gw)
        if violation_gw > tolerance_gw:
            violating_provinces.append(pro)
        by_province[pro] = {
            "a1_fixed_plus_flexible_peak_gw": peak_gw,
            "peak_hour_zero_based": peak_hour,
            "destination_power_cap_gw": cap_gw,
            "minimum_margin_gw": float(cap_gw - peak_gw),
            "max_violation_gw": float(violation_gw),
        }

    status = (
        "PASS_STRICT"
        if max_violation_gw <= tolerance_gw
    ...
    audit = {
        ...
        "max_violation_gw": float(max_violation_gw),
        "violating_provinces": violating_provinces,
        "by_province": by_province,
        "interpretation": (
            "PASS_STRICT confirms that the existing A1 fixed-origin hourly AI "
            "trajectory is feasible under the physical destination power caps "
            "used by A3, avoiding a hidden host-cap comparison mismatch."
        ),
    }
    if status != "PASS_STRICT":
        raise ValueError(
            "Existing A1 is incompatible with the A3 physical host-cap contract: "
            f"max_violation_gw={max_violation_gw}, "
            f"violating_provinces={violating_provinces}."
        )
    return audit
...
def National_energy_model(args):
    ...
    requested_ai_operational_scenario = str(
        _get_arg(
            args,
            "ai_operational_scenario",
            os.environ.get("AI_OPERATIONAL_SCENARIO", "S4"),
        )
    ).strip().upper()
    if requested_ai_operational_scenario not in AI_OPERATIONAL_SCENARIOS:
        raise ValueError(
            "Main manuscript AI scenarios are restricted to S0, S1 and S4. "
            f"Got {requested_ai_operational_scenario!r}. S2/S3 have been removed "
            "from the main model."
        )
    # This dedicated file always reuses the S4 operational machinery. The
    # long-term planning layer is disabled independently below by fixing every
    # OD siting/runtime variable to its S0-origin target.
    ai_operational_scenario = "S4"
    ...
    rt_only_zero_shift_a1_parity_audit = None
    rt_only_a1_reference_host_cap_audit = None
    s0_use_cluster_reconstruction = str_to_bool(
        os.environ.get("S0_USE_CLUSTER_RECONSTRUCTION"), default=True
    )
    requested_strict_cluster_ai_interface = str_to_bool(
        os.environ.get("STRICT_CLUSTER_AI_INTERFACE", "true"), default=True
    )
    strict_cluster_ai_interface = True
    requested_strict_postsolve_ai_audit = str_to_bool(
        os.environ.get("STRICT_POSTSOLVE_AI_AUDIT", "true"), default=True
    )
    strict_postsolve_ai_audit = True
    requested_raise_on_structural_ai_audit_fail = str_to_bool(
        os.environ.get("RAISE_ON_STRUCTURAL_AI_AUDIT_FAIL", "true"),
        default=True,
    )
    raise_on_structural_ai_audit_fail = True
    ...
    LOAD_DEMAND = {}
    requested_use_external_ai_load = str_to_bool(
        os.environ.get("USE_EXTERNAL_AI_LOAD"), default=False
    )
    use_external_ai_load = True
    ...
    requested_ai_s4_zone_runtime_transfer = str_to_bool(
        os.environ.get("AI_S4_ZONE_RUNTIME_TRANSFER"), default=False
    )
    requested_ai_rt_zone_route_mode = str(
        os.environ.get("AI_RT_ZONE_ROUTE_MODE", "adjacent")
    ).strip().lower()
    ai_s4_zone_runtime_transfer = True
    ai_rt_zone_route_mode = "adjacent"
    ...
    requested_ai_zone_neighbors_mode = str(
        os.environ.get("AI_ZONE_NEIGHBORS", "adjacent")
    ).strip().lower()
    ai_zone_neighbors_mode = "adjacent"
    ...
    if use_external_ai_load:
        ...
        if os.path.isdir(ai_load_file):
            ...
            if not ai_interface_dual_pool:
                raise ValueError(
                    "RT-only requires a dual-pool source-cluster AI interface "
                    "with an explicit runtime subset."
                )
            if ai_interface_runtime_pool_gw <= 0.0:
                raise ValueError(
                    "RT-only requires a strictly positive runtime_pool_gw."
                )
            if ai_interface_runtime_pool_gw > ai_interface_siting_pool_gw + 1e-9:
                raise ValueError(
                    "RT-only runtime_pool_gw must be a subset of siting_pool_gw: "
                    f"runtime={ai_interface_runtime_pool_gw}, "
                    f"siting={ai_interface_siting_pool_gw}."
                )
        else:
        ...
        if use_source_cluster_ai_interface:
            ...
            ai_s4_od_params = _build_s4_od_parameters(
                ...
                zone_neighbors=zone_neighbors,
                rt_only_fixed_origin_mode=True,
            )
            ai_s4_od_params = configure_rt_only_fixed_origin_parameters(
            ...
            rt_only_zero_shift_a1_parity_audit = (
                audit_rt_only_zero_shift_a1_parity(
                    ai_cluster_params=ai_s4_od_params,
                    s0_cluster_fixed_flexible_load_mw=(
                        s0_cluster_fixed_flexible_load_mw
                    ),
            ...
            ai_s4_od_params["rt_only_zero_shift_a1_parity_audit"] = dict(
                rt_only_zero_shift_a1_parity_audit
            )
            ai_s4_od_params["audit"]["rt_only_zero_shift_a1_parity"] = dict(
                rt_only_zero_shift_a1_parity_audit
            )
            ...
            s0_reconstruction_audit["applied_to_load_demand"] = False
            s0_reconstruction_audit[
                "applied_as_rt_only_planning_anchor"
            ] = True
        ...
        ai_interface_metadata["rt_only_model_variant"] = {
            "enabled": bool(RT_ONLY_MODEL_VARIANT),
            "counterfactual": RT_ONLY_COUNTERFACTUAL,
            "planning_enabled": False,
            "runtime_enabled": True,
            "planning_anchor": RT_ONLY_PLANNING_ANCHOR,
            "runtime_profile_mode": ai_s4_od_params.get("runtime_profile_mode"),
            "expected_runtime_profile_mode": RT_ONLY_RUNTIME_PROFILE_MODE,
            "runtime_profile_effective_max_abs_diff": ai_s4_od_params.get(
                "runtime_profile_effective_max_abs_diff"
            ),
            "runtime_profile_input_max_abs_diff": ai_s4_od_params.get(
                "runtime_profile_input_max_abs_diff"
            ),
            "requested_ai_operational_scenario": requested_ai_operational_scenario,
            "internal_ai_operational_scenario": ai_operational_scenario,
            "requested_use_external_ai_load": bool(
                requested_use_external_ai_load
            ),
            "effective_use_external_ai_load": bool(use_external_ai_load),
            "requested_strict_cluster_ai_interface": bool(
                requested_strict_cluster_ai_interface
            ),
            "effective_strict_cluster_ai_interface": bool(
                strict_cluster_ai_interface
            ),
            "requested_strict_postsolve_ai_audit": bool(
                requested_strict_postsolve_ai_audit
            ),
            "effective_strict_postsolve_ai_audit": bool(
                strict_postsolve_ai_audit
            ),
            "requested_raise_on_structural_ai_audit_fail": bool(
                requested_raise_on_structural_ai_audit_fail
            ),
            "effective_raise_on_structural_ai_audit_fail": bool(
                raise_on_structural_ai_audit_fail
            ),
            "requested_zone_runtime_transfer": bool(
                requested_ai_s4_zone_runtime_transfer
            ),
            "effective_zone_runtime_transfer": bool(
                ai_s4_zone_runtime_transfer
            ),
            "requested_runtime_zone_route_mode": requested_ai_rt_zone_route_mode,
            "effective_runtime_zone_route_mode": ai_rt_zone_route_mode,
            "requested_zone_neighbors_mode": requested_ai_zone_neighbors_mode,
            "effective_zone_neighbors_mode": ai_zone_neighbors_mode,
            "effective_ai_scenario": RT_ONLY_EFFECTIVE_SCENARIO,
            "note": (
                "Long-term siting and the runtime subset are fixed to the S0 "
                "source-cluster origin reconstruction. The runtime subset uses the "
                "same smoothed arrival profile as A1; only adjacent-zone runtime "
                "routing and deadline-constrained hourly execution remain optimized."
            ),
        }
        ...
        if use_source_cluster_ai_interface:
            ...
            ai_interface_metadata["rt_only_fixed_origin_parameter_audit"] = dict(
                ai_s4_od_params["rt_only_fixed_origin_audit"]
            )
            ai_interface_metadata["rt_only_zero_shift_a1_parity_audit"] = dict(
                rt_only_zero_shift_a1_parity_audit
            )
            ai_interface_metadata["rt_only_fixed_origin_targets_gw"] = {
                "siting_by_cluster_and_origin": ai_s4_od_params[
                    "rt_only_fixed_siting_target_gw"
                ],
                "runtime_by_cluster_and_origin": ai_s4_od_params[
                    "rt_only_fixed_runtime_target_gw"
                ],
                "siting_by_origin_province": ai_s4_od_params[
                    "rt_only_fixed_destination_siting_gw"
                ],
                "runtime_by_origin_province": ai_s4_od_params[
                    "rt_only_fixed_destination_runtime_gw"
                ],
            }
        ...
        ai_interface_metadata["requested_ai_operational_scenario"] = (
            requested_ai_operational_scenario
        )
        ...
        ai_interface_metadata["scenario_definitions"] = {
            ...
            "A3_RUNTIME_ONLY": (
                "Planning-off/runtime-on counterfactual: long-term siting and the "
                "runtime subset are fixed to the S0 source-cluster origin weights; "
                "the runtime subset may be routed to adjacent execution zones and "
                "shifted within its deadline window."
            ),
        }
        ...
        if RT_ONLY_MODEL_VARIANT:
            effective_ai_scenario = RT_ONLY_EFFECTIVE_SCENARIO
        elif ai_operational_scenario == "S0":
        ...
        if is_cluster_od_interface:
            ...
            for zone, pool_gw in sorted(zone_cluster_pool_gw.items()):
                ...
                if (not RT_ONLY_MODEL_VARIANT) and cap_gw + 1e-6 < pool_gw:
                    raise ValueError(
            ...
            for zone, pool_gw in sorted(zone_cluster_pool_gw.items()):
                ...
                if (
                    not RT_ONLY_MODEL_VARIANT
                    and use_network_tier_caps
            ...
            if (
                not RT_ONLY_MODEL_VARIANT
                and od_active_energy_avg_cap_sum + 1e-6 < hourly_AI_load
            ):
                raise ValueError(
            ...
            rt_only_a1_reference_host_cap_audit = (
                audit_rt_only_a1_reference_host_cap(
                    fixed_ai_load=fixed_ai_load,
                    s0_cluster_fixed_flexible_load_mw=(
                        s0_cluster_fixed_flexible_load_mw
                    ),
            ...
            ai_interface_metadata["rt_only_a1_reference_host_cap_audit"] = dict(
                rt_only_a1_reference_host_cap_audit
            )
            rt_only_fixed_immediate_power_cap_audit = {}
            for pro in Province:
                pro = str(pro)
                incoming_clusters = ai_s4_od_params["clusters_by_destination"].get(
                    pro, []
                )
                cap_gw = float(ai_s4_od_params["destination_power_cap_gw"][pro])
                max_fixed_immediate_gw = 0.0
                for h in range(HOURS):
                    fixed_ai_gw = float(
                        fixed_ai_load.get(pro, np.zeros(HOURS))[h]
                    ) / 1000.0
                    siting_only_gw = sum(
                        (
                            float(
                    ...
                    max_fixed_immediate_gw = max(
                        max_fixed_immediate_gw,
                        fixed_ai_gw + siting_only_gw,
                    )
                margin_gw = cap_gw - max_fixed_immediate_gw
                rt_only_fixed_immediate_power_cap_audit[pro] = {
                    "fixed_ai_plus_siting_only_peak_gw": float(
                        max_fixed_immediate_gw
                    ),
                    "destination_power_cap_gw": cap_gw,
                    "margin_for_runtime_execution_gw": float(margin_gw),
                }
                if margin_gw < -1e-6:
                    raise ValueError(
                        "RT-only fixed-origin non-runtime load violates the physical "
                        f"destination power cap for {pro}: peak="
                        f"{max_fixed_immediate_gw}, cap={cap_gw}."
                    )
            ai_interface_metadata[
                "rt_only_fixed_immediate_power_cap_audit"
            ] = rt_only_fixed_immediate_power_cap_audit
            ai_interface_metadata["rt_only_nonbinding_planning_cap_note"] = (
                "Destination energy, hosting-share and combined planning caps are "
                "reported only as diagnostics in A3. They cannot move the fixed "
                "S0-origin siting anchor. Hourly physical destination power caps "
                "remain binding for execution."
            )
            if is_s4_od_interface:
    ...
    if use_source_cluster_ai_interface and ai_operational_scenario in {"S1", "S4"}:
        ...
        for gid in ai_s4_od_params["source_clusters"]:
            ...
            # A3 has no planning recourse: unplaced siting/runtime workload is
            # structurally forbidden rather than penalized through slack.
            ...
            for dest in allowed_destinations_by_cluster[gid]:
                fixed_siting_gw = float(
                    ai_s4_od_params["rt_only_fixed_siting_target_gw"][gid][dest]
                )
                fixed_runtime_gw = float(
                    ai_s4_od_params["rt_only_fixed_runtime_target_gw"][gid][dest]
                )
        ...
        for gid in ai_s4_od_params["source_clusters"]:
            ...
            total_arc_ub = float(
                sum(
                    ai_s4_od_params["rt_only_fixed_siting_target_gw"][gid].values()
                )
            )
            capacity_ratio = (
        ...
        cluster_od_variable_status = {
            "AI_OD_FLOW": {
                ...
                "arc_policy": (
                    "fixed_s0_origin_arcs_only"
                ),
                "decision_status": "fixed_parameter_carrier_not_planning_decision",
            },
            "AI_BATCH_RUN": {
    ...
    for pro in Province:
        ...
        if AI_OD_FLOW:
            fixed_pro_ai_load_gw = float(
                ai_s4_od_params["rt_only_fixed_destination_siting_gw"].get(
                    str(pro), 0.0
                )
            )
            pro_ai_load_lb = fixed_pro_ai_load_gw
            pro_ai_load_ub = fixed_pro_ai_load_gw
    ...
    if use_external_ai_load:
        ...
        if ai_operational_scenario == "S4":
            ...
            s4_od_planning_status = add_rt_only_fixed_origin_constraints(
                model=model,
                AI_OD_FLOW=AI_OD_FLOW,
                PRO_AI_LOAD=PRO_AI_LOAD,
                ai_cluster_params=ai_s4_od_params,
            ...
            s4_destination_network_share_status = {
                ...
                "planning_enabled": False,
                "reason": (
                    "A3 Runtime-only fixes long-term siting to S0 origins; "
                    "hosting-share planning caps are non-binding diagnostics."
                ),
            }
    ...
    for pro in Province:
        ...
        if use_external_ai_load:
            if ai_operational_scenario == "S0":
                ...
                incoming_clusters = ai_s4_od_params[
                    "clusters_by_destination"
                ].get(str(pro), [])
                siting_only_avg_gw_expr = gp.quicksum(
                    AI_OD_FLOW[gid][str(pro)] - AI_RT_OD_FLOW[gid][str(pro)]
                    for gid in incoming_clusters
                )
                runtime_execution_avg_gw_expr = (
                    gp.quicksum(AI_BATCH_RUN[pro][h] for h in range(HOURS))
                    / HOURS
                )
                flexible_avg_gw_expr = (
                    siting_only_avg_gw_expr + runtime_execution_avg_gw_expr
                )
            else:
                flexible_avg_gw_expr = 0.0
        else:
    ...
    if RT_ONLY_MODEL_VARIANT:
        # Materialize newly added variables before inspecting their bounds.
        model.update()
        structural_errors = []
        if not use_external_ai_load:
            structural_errors.append("external AI interface is not enabled")
        if not use_source_cluster_ai_interface:
            structural_errors.append("source-cluster interface is not loaded")
        if not ai_interface_dual_pool:
            structural_errors.append("dual-pool interface is not active")
        if ai_operational_scenario != "S4":
            structural_errors.append("internal S4 runtime machinery is not active")
        if not ai_s4_zone_runtime_transfer_active:
            structural_errors.append("zone runtime transfer is not active")
        if ai_rt_zone_route_mode != "adjacent":
            structural_errors.append("runtime route mode is not adjacent")
        if not AI_OD_FLOW:
            structural_errors.append("fixed siting OD variables are missing")
        if not AI_RT_OD_FLOW:
            structural_errors.append("fixed runtime OD variables are missing")
        if not AI_RT_ZONE_ROUTE:
            structural_errors.append("runtime zone-route variables are missing")
        if not AI_BATCH_CUM:
            structural_errors.append("runtime batch cumulative variables are missing")
        if ai_s4_od_params.get("cross_zone_arcs"):
            structural_errors.append("planning cross-zone arcs are present")
        runtime_profile_mode = ai_s4_od_params.get("runtime_profile_mode")
        runtime_profile_effective_max_abs_diff = float(
            ai_s4_od_params.get(
                "runtime_profile_effective_max_abs_diff", float("inf")
            )
        )
        zero_shift_parity_status = ai_s4_od_params.get(
            "rt_only_zero_shift_a1_parity_audit", {}
        ).get("status")
        a1_reference_host_cap_status = ai_interface_metadata.get(
            "rt_only_a1_reference_host_cap_audit", {}
        ).get("status")
        if runtime_profile_mode != RT_ONLY_RUNTIME_PROFILE_MODE:
            structural_errors.append(
                "runtime profile does not match the A1 smoothed profile contract"
            )
        if runtime_profile_effective_max_abs_diff > 1e-12:
            structural_errors.append(
                "effective runtime and A1 cluster profiles differ"
            )
        if zero_shift_parity_status != "PASS_STRICT":
            structural_errors.append(
                "zero-shift A3 does not reproduce A1 hourly flexible load"
            )
        if a1_reference_host_cap_status != "PASS_STRICT":
            structural_errors.append(
                "A1 reference trajectory is incompatible with A3 host caps"
            )

        fixed_siting_targets = ai_s4_od_params.get(
            "rt_only_fixed_siting_target_gw", {}
        )
        fixed_runtime_targets = ai_s4_od_params.get(
            "rt_only_fixed_runtime_target_gw", {}
        )
        for gid, by_destination in fixed_siting_targets.items():
            if set(AI_OD_FLOW.get(gid, {})) != set(by_destination):
                structural_errors.append(
                    f"fixed siting arc set mismatch for cluster {gid}"
                )
                continue
            if set(AI_RT_OD_FLOW.get(gid, {})) != set(
                fixed_runtime_targets.get(gid, {})
            ):
                structural_errors.append(
                    f"fixed runtime arc set mismatch for cluster {gid}"
                )
                continue
            for dest, target in by_destination.items():
                siting_var = AI_OD_FLOW[gid][dest]
                runtime_var = AI_RT_OD_FLOW[gid][dest]
                runtime_target = float(fixed_runtime_targets[gid][dest])
                if (
                    abs(float(siting_var.LB) - float(target)) > 1e-10
                    or abs(float(siting_var.UB) - float(target)) > 1e-10
                ):
                    structural_errors.append(
                        f"siting variable is not fixed to target for {gid}->{dest}"
                    )
                if (
                    abs(float(runtime_var.LB) - runtime_target) > 1e-10
                    or abs(float(runtime_var.UB) - runtime_target) > 1e-10
                ):
                    structural_errors.append(
                        f"runtime variable is not fixed to target for {gid}->{dest}"
                    )
        if any(float(var.UB) != 0.0 for var in AI_UNMET_SITING.values()):
            structural_errors.append("siting unmet slack is not fixed to zero")
        if any(float(var.UB) != 0.0 for var in AI_UNMET_RUNTIME.values()):
            structural_errors.append("runtime unmet slack is not fixed to zero")
        if migration_cost:
            structural_errors.append("planning migration-cost terms are present")

        ai_interface_metadata["rt_only_structural_precheck"] = {
            "status": "PASS_STRICT" if not structural_errors else "FAIL_STRUCTURAL",
            ...
            "zero_shift_a1_parity_status": zero_shift_parity_status,
            "a1_reference_host_cap_status": a1_reference_host_cap_status,
            "fixed_siting_arc_count": int(
                sum(len(v) for v in AI_OD_FLOW.values())
            ),
            "fixed_runtime_arc_count": int(
                sum(len(v) for v in AI_RT_OD_FLOW.values())
            ),
            "runtime_zone_route_variable_count": int(
                sum(
                    len(by_exec_zone)
                    for by_site_zone in AI_RT_ZONE_ROUTE.values()
                    for by_exec_zone in by_site_zone.values()
                )
            ),
        }
        if structural_errors:
            raise RuntimeError(
                "RT-only structural precheck failed: " + "; ".join(structural_errors)
            )
    ...
    if model.status == gp.GRB.INFEASIBLE:
        ...
        model.computeIIS()
        failure_res_dir = os.path.join(args.output_dir, args.Mode)
        os.makedirs(failure_res_dir, exist_ok=True)
        iis_path = os.path.join(failure_res_dir, "model_iis.ilp")
        model.write(iis_path)
        print(f"Irreducible Inconsistent Subsystem (IIS) written to {iis_path}")
        ...
        model.optimize()
        print(
            f"[SOLVER] status after DualReductions=0: {int(model.status)} "
            f"({_status_name_map.get(model.status, 'OTHER')}), "
            f"SolCount={model.SolCount}"
        )
        if model.status == gp.GRB.INFEASIBLE:
    ...
    if has_usable_solution and model.SolCount > 0:
        ...
        if AI_OD_FLOW:
            ...
            for gid in AI_OD_FLOW:
                if RT_ONLY_MODEL_VARIANT:
                    # In A3 each fixed cluster->destination carrier is already
                    # the S0-origin share. Re-applying the cluster origin weight
                    # here would double-weight and fabricate migration.
                    for origin in AI_OD_FLOW[gid]:
                        flow_gw = float(AI_OD_FLOW[gid][origin].x)
                        results_AI_implied_province_od.setdefault(origin, {})
                        results_AI_implied_province_od[origin][origin] = (
                            results_AI_implied_province_od[origin].get(origin, 0.0)
                            + flow_gw
                        )
                else:
            ...
            ai_interface_metadata["implied_province_od_postprocess"] = {
                "enabled": True,
                "method": (
                    "RT-only identity mapping: each fixed cluster-to-origin carrier "
                    "is reported as origin-to-same-origin flow; no planning migration "
                    "is reconstructed."
                    if RT_ONLY_MODEL_VARIANT
                    else "Y_hat[p,d] = origin_reconstruction_weight[p,g] * Y[g,d]; "
        ...
        if cluster_od_solution_audit:
            ...
            if ai_operational_scenario == "S4":
                ...
                ai_interface_metadata["s1_od_solution_audit"] = cluster_od_solution_audit
        rt_only_tol_gw = float(os.environ.get("RT_ONLY_AUDIT_TOL_GW", "1e-6"))
        rt_only_route_total_abs_tol_gw = float(
            os.environ.get("RT_ONLY_ROUTE_TOTAL_ABS_TOL_GW", "1e-5")
        )
        rt_only_route_total_rel_tol = float(
            os.environ.get("RT_ONLY_ROUTE_TOTAL_REL_TOL", "1e-7")
        )
        if (
            not np.isfinite(rt_only_route_total_abs_tol_gw)
            or rt_only_route_total_abs_tol_gw < 0.0
            or not np.isfinite(rt_only_route_total_rel_tol)
            or rt_only_route_total_rel_tol < 0.0
        ):
            raise ValueError(
                "RT-only aggregate route-total tolerances must be finite and "
                "non-negative."
            )
        fixed_siting_targets = ai_s4_od_params.get(
            "rt_only_fixed_siting_target_gw", {}
        ...
        actual_siting_arcs = {
            (gid, dest)
            for gid, by_destination in AI_OD_FLOW.items()
            for dest in by_destination
        }
        actual_runtime_arcs = {
            (gid, dest)
            for gid, by_destination in AI_RT_OD_FLOW.items()
            for dest in by_destination
        }
        extra_siting_arcs = sorted(actual_siting_arcs - expected_fixed_arcs)
        missing_siting_arcs = sorted(expected_fixed_arcs - actual_siting_arcs)
        extra_runtime_arcs = sorted(actual_runtime_arcs - expected_fixed_arcs)
        missing_runtime_arcs = sorted(expected_fixed_arcs - actual_runtime_arcs)
        fixed_siting_max_abs_gap_gw = 0.0
        fixed_runtime_max_abs_gap_gw = 0.0
        fixed_siting_free_variable_count = 0
        fixed_runtime_free_variable_count = 0
        for gid, dest in expected_fixed_arcs:
            if dest in AI_OD_FLOW.get(gid, {}):
                fixed_siting_max_abs_gap_gw = max(
                    fixed_siting_max_abs_gap_gw,
                    abs(
                        float(AI_OD_FLOW[gid][dest].x)
                        - float(fixed_siting_targets[gid][dest])
                    ),
                )
                fixed_siting_free_variable_count += int(
                    abs(
                        float(AI_OD_FLOW[gid][dest].UB)
                        - float(AI_OD_FLOW[gid][dest].LB)
                    )
                    > rt_only_tol_gw
                )
            if dest in AI_RT_OD_FLOW.get(gid, {}):
                fixed_runtime_max_abs_gap_gw = max(
                    fixed_runtime_max_abs_gap_gw,
                    abs(
                        float(AI_RT_OD_FLOW[gid][dest].x)
                        - float(fixed_runtime_targets[gid][dest])
                    ),
                )
                fixed_runtime_free_variable_count += int(
                    abs(
                        float(AI_RT_OD_FLOW[gid][dest].UB)
                        - float(AI_RT_OD_FLOW[gid][dest].LB)
                    )
                    > rt_only_tol_gw
                )
        fixed_destination_siting_target = ai_s4_od_params.get(
            "rt_only_fixed_destination_siting_gw", {}
        )
        fixed_destination_siting_max_abs_gap_gw = max(
            (
                abs(
                    float(PRO_AI_LOAD[pro].x)
                    - float(fixed_destination_siting_target.get(str(pro), 0.0))
                )
                for pro in Province
            ),
            default=0.0,
        )
        non_origin_positive_siting_flow_gw = float(
            sum(
                abs(float(AI_OD_FLOW[gid][dest].x))
                for gid, dest in extra_siting_arcs
            )
        )
        non_origin_positive_runtime_flow_gw = float(
            sum(
                abs(float(AI_RT_OD_FLOW[gid][dest].x))
                for gid, dest in extra_runtime_arcs
            )
        )
        unmet_siting_total_gw = float(
            sum(float(var.x) for var in AI_UNMET_SITING.values())
        )
        unmet_runtime_total_gw = float(
            sum(float(var.x) for var in AI_UNMET_RUNTIME.values())
        )
        planning_migration_cost_myuan = float(
            cluster_od_solution_audit.get("migration_cost_total_myuan", 0.0) or 0.0
        )
        runtime_route_invalid_arc_count = 0
        runtime_route_variable_count = 0
        zone_neighbors_for_runtime = ai_s4_od_params.get("zone_neighbors") or {}
        for gid, by_site_zone in AI_RT_ZONE_ROUTE.items():
            for site_zone, by_exec_zone in by_site_zone.items():
                allowed_exec_zones = set(
                    zone_neighbors_for_runtime.get(site_zone, {site_zone})
                )
                for exec_zone in by_exec_zone:
                    runtime_route_variable_count += 1
                    if exec_zone not in allowed_exec_zones:
                        runtime_route_invalid_arc_count += 1
        runtime_pool_target_gw = float(
            ai_s4_od_params.get("runtime_pool_gw", 0.0)
        )
        runtime_route_total_gw = float(
            ai_interface_metadata.get(
                "s4_zone_runtime_transfer_solution_audit", {}
            ).get("runtime_zone_route_total_gw", 0.0)
            or 0.0
        )
        runtime_route_total_abs_gap_gw = abs(
            runtime_route_total_gw - runtime_pool_target_gw
        )
        runtime_route_total_relative_gap = (
            runtime_route_total_abs_gap_gw / abs(runtime_pool_target_gw)
            if abs(runtime_pool_target_gw) > 1e-12
            else runtime_route_total_abs_gap_gw
        )
        runtime_route_total_allowed_gap_gw = max(
            rt_only_route_total_abs_tol_gw,
            rt_only_route_total_rel_tol * abs(runtime_pool_target_gw),
        )
        runtime_execution_target_gwh = float(runtime_pool_target_gw * HOURS)
        runtime_execution_abs_gap_gwh = abs(
            float(batch_run_total_gwh) - runtime_execution_target_gwh
        )
        zero_shift_a1_parity_audit = ai_interface_metadata.get(
            "rt_only_zero_shift_a1_parity_audit", {}
        )
        a1_reference_host_cap_audit = ai_interface_metadata.get(
            "rt_only_a1_reference_host_cap_audit", {}
        )
        rt_only_checks = {
            "variant_enabled": bool(RT_ONLY_MODEL_VARIANT),
            "external_ai_interface": bool(use_external_ai_load),
            "source_cluster_interface": bool(use_source_cluster_ai_interface),
            "strict_cluster_interface": bool(strict_cluster_ai_interface),
            "dual_pool_interface": bool(ai_interface_dual_pool),
            "internal_s4_runtime_machinery": ai_operational_scenario == "S4",
            "planning_disabled": True,
            "runtime_enabled": bool(
                ai_s4_zone_runtime_transfer_active
                and AI_BATCH_RUN
                and AI_BATCH_CUM
                and AI_RT_ZONE_ROUTE
            ),
            "adjacent_runtime_route_mode": ai_rt_zone_route_mode == "adjacent",
            "a1_smoothed_runtime_profile_contract": bool(
                ai_s4_od_params.get("runtime_profile_mode")
                == RT_ONLY_RUNTIME_PROFILE_MODE
                and float(
                    ai_s4_od_params.get(
                        "runtime_profile_effective_max_abs_diff", float("inf")
            ...
            "zero_shift_a1_hourly_parity": (
                zero_shift_a1_parity_audit.get("status") == "PASS_STRICT"
            ),
            "a1_reference_host_cap_compatible": (
                a1_reference_host_cap_audit.get("status") == "PASS_STRICT"
            ),
            "fixed_siting_arc_set_exact": bool(
                not extra_siting_arcs and not missing_siting_arcs
            ),
            "fixed_runtime_arc_set_exact": bool(
                not extra_runtime_arcs and not missing_runtime_arcs
            ),
            "fixed_siting_variables_not_free": fixed_siting_free_variable_count == 0,
            "fixed_runtime_variables_not_free": fixed_runtime_free_variable_count == 0,
            "fixed_siting_target_within_tolerance": (
                fixed_siting_max_abs_gap_gw <= rt_only_tol_gw
            ),
            "fixed_runtime_target_within_tolerance": (
                fixed_runtime_max_abs_gap_gw <= rt_only_tol_gw
            ),
            "fixed_destination_target_within_tolerance": (
                fixed_destination_siting_max_abs_gap_gw <= rt_only_tol_gw
            ),
            "non_origin_siting_flow_zero": (
                non_origin_positive_siting_flow_gw <= rt_only_tol_gw
            ),
            "non_origin_runtime_flow_zero": (
                non_origin_positive_runtime_flow_gw <= rt_only_tol_gw
            ),
            "unmet_siting_zero": unmet_siting_total_gw <= rt_only_tol_gw,
            "unmet_runtime_zero": unmet_runtime_total_gw <= rt_only_tol_gw,
            "planning_migration_cost_zero": abs(planning_migration_cost_myuan) <= 1e-9,
            "runtime_route_variables_present": runtime_route_variable_count > 0,
            "runtime_route_arcs_self_or_adjacent": runtime_route_invalid_arc_count == 0,
            "runtime_route_total_conserved": (
                runtime_route_total_abs_gap_gw
                <= runtime_route_total_allowed_gap_gw
            ),
            "runtime_execution_energy_conserved": (
                runtime_execution_abs_gap_gwh <= ai_energy_audit_abs_tol_gwh
            ),
            "no_advance_within_tolerance": (
                float(no_advance) <= ai_temporal_audit_tol_gwh
            ),
            "deadline_within_tolerance": (
                float(deadline) <= ai_temporal_audit_tol_gwh
            ),
            "destination_power_cap_within_tolerance": (
                destination_power_cap_violation_gw <= ai_power_audit_tol_gw
            ),
            "ai_service_energy_conserved": bool(
                ai_execution_abs_gap_gwh <= ai_energy_audit_abs_tol_gwh
                and ai_execution_relative_abs_gap <= ai_energy_audit_rel_tol
            ),
            "structural_precheck_passed": (
                ai_interface_metadata.get("rt_only_structural_precheck", {}).get(
                    "status"
                )
                == "PASS_STRICT"
            ),
        }
        rt_only_failed_checks = sorted(
            key for key, passed in rt_only_checks.items() if not passed
        )
        rt_only_audit_status = (
            "PASS_STRICT" if not rt_only_failed_checks else "FAIL_STRUCTURAL"
        )
        rt_only_audit = {
            "status": rt_only_audit_status,
            "counterfactual": RT_ONLY_COUNTERFACTUAL,
            "effective_ai_scenario": RT_ONLY_EFFECTIVE_SCENARIO,
            "planning_enabled": False,
            "runtime_enabled": True,
            "planning_anchor": RT_ONLY_PLANNING_ANCHOR,
            "comparison_scope": (
                "A1 fixed-AI to A3 runtime-only at fixed S0-origin siting; "
                "not a complete A1-A4 factorial interaction estimate"
            ),
            "runtime_profile_mode": ai_s4_od_params.get("runtime_profile_mode"),
            "expected_runtime_profile_mode": RT_ONLY_RUNTIME_PROFILE_MODE,
            "runtime_profile_input_max_abs_diff": float(
                ai_s4_od_params.get("runtime_profile_input_max_abs_diff", 0.0)
            ),
            "runtime_profile_effective_max_abs_diff": float(
                ai_s4_od_params.get(
                    "runtime_profile_effective_max_abs_diff", float("inf")
            ...
            "zero_shift_a1_parity_status": zero_shift_a1_parity_audit.get(
                "status"
            ),
            "zero_shift_a1_max_abs_gap_mw": zero_shift_a1_parity_audit.get(
                "max_abs_gap_mw"
            ),
            "a1_reference_host_cap_status": a1_reference_host_cap_audit.get(
                "status"
            ),
            "a1_reference_host_cap_max_violation_gw": (
                a1_reference_host_cap_audit.get("max_violation_gw")
            ),
            "requested_ai_operational_scenario": requested_ai_operational_scenario,
            "internal_ai_operational_scenario": ai_operational_scenario,
            "runtime_zone_route_mode": ai_rt_zone_route_mode,
            "tolerance_gw": float(rt_only_tol_gw),
            "aggregate_route_total_abs_tolerance_gw": float(
                rt_only_route_total_abs_tol_gw
            ),
            "aggregate_route_total_relative_tolerance": float(
                rt_only_route_total_rel_tol
            ),
            "checks": rt_only_checks,
            "failed_checks": rt_only_failed_checks,
            "expected_fixed_arc_count": int(len(expected_fixed_arcs)),
            "extra_siting_arcs": [list(item) for item in extra_siting_arcs],
            "missing_siting_arcs": [list(item) for item in missing_siting_arcs],
            "extra_runtime_arcs": [list(item) for item in extra_runtime_arcs],
            "missing_runtime_arcs": [list(item) for item in missing_runtime_arcs],
            "fixed_siting_free_variable_count": int(
                fixed_siting_free_variable_count
            ),
            "fixed_runtime_free_variable_count": int(
                fixed_runtime_free_variable_count
            ),
            "fixed_siting_max_abs_gap_gw": float(
                fixed_siting_max_abs_gap_gw
            ),
            "fixed_runtime_max_abs_gap_gw": float(
                fixed_runtime_max_abs_gap_gw
            ),
            "fixed_destination_siting_max_abs_gap_gw": float(
                fixed_destination_siting_max_abs_gap_gw
            ),
            "non_origin_positive_siting_flow_gw": (
                non_origin_positive_siting_flow_gw
            ),
            "non_origin_positive_runtime_flow_gw": (
                non_origin_positive_runtime_flow_gw
            ),
            "unmet_siting_total_gw": unmet_siting_total_gw,
            "unmet_runtime_total_gw": unmet_runtime_total_gw,
            "planning_migration_cost_myuan": planning_migration_cost_myuan,
            "runtime_route_variable_count": int(runtime_route_variable_count),
            "runtime_route_invalid_arc_count": int(
                runtime_route_invalid_arc_count
            ),
            "runtime_pool_target_gw": runtime_pool_target_gw,
            "runtime_route_total_gw": runtime_route_total_gw,
            "runtime_route_total_abs_gap_gw": runtime_route_total_abs_gap_gw,
            "runtime_route_total_relative_gap": float(
                runtime_route_total_relative_gap
            ),
            "runtime_route_total_allowed_gap_gw": float(
                runtime_route_total_allowed_gap_gw
            ),
            "runtime_route_total_tolerance_note": (
                "The aggregate route total sums many solver variables and uses a "
                "dedicated max(absolute, relative) numerical tolerance. All fixed "
                "planning, unmet-load and physical-cap checks retain their stricter "
                "individual tolerances."
            ),
            "runtime_execution_target_gwh": runtime_execution_target_gwh,
            "runtime_execution_total_gwh": float(batch_run_total_gwh),
            "runtime_execution_abs_gap_gwh": runtime_execution_abs_gap_gwh,
            "no_advance_max_violation_gwh": float(no_advance),
            "deadline_max_violation_gwh": float(deadline),
            "destination_power_cap_max_violation_gw": float(
                destination_power_cap_violation_gw
            ),
            "ai_service_energy_abs_gap_gwh": float(ai_execution_abs_gap_gwh),
            "ai_service_energy_relative_abs_gap": float(
                ai_execution_relative_abs_gap
            ),
            "interpretation": (
                "PASS_STRICT proves that long-term siting is fixed to S0 origins "
                "and that only the deadline-constrained adjacent-zone runtime layer "
                "remains operational. It also proves zero-shift hourly parity with "
                "A1 under a shared smoothed arrival profile and compatible physical "
                "host caps. A non-zero cross-zone runtime route is allowed but not "
                "required for a valid optimum."
            ),
        }
        ai_interface_metadata["rt_only_audit"] = rt_only_audit
        ...
        ai_audit_usable_for_main_analysis = overall_ai_audit_status in {
            "PASS_STRICT",
            "WARN_NUMERICAL_RESIDUAL",
        }
        solver_strict_for_main_analysis = model.status == GRB.OPTIMAL
        usable_for_main_analysis = bool(
            ai_audit_usable_for_main_analysis
            and solver_strict_for_main_analysis
        )
        hard_failed = overall_ai_audit_status == "FAIL_STRUCTURAL"
        requires_manual_review = bool(
            overall_ai_audit_status == "WARN_REVIEW_REQUIRED"
            or (not solver_strict_for_main_analysis and not hard_failed)
        )
        ai_execution_audit = {
            ...
            "rt_only_audit_status": rt_only_audit_status,
            "rt_only_failed_checks": rt_only_failed_checks,
            "solver_strict_for_main_analysis": bool(
                solver_strict_for_main_analysis
            ),
        ...
        with open(
            os.path.join(res_dir, "rt_only_audit.json"),
            "w",
            encoding="utf-8",
        ) as f:
            json.dump(rt_only_audit, f, indent=2, ensure_ascii=False)
        if ai_s4_zone_runtime_transfer_active:
            with open(
        ...
        required_result_files = [
            os.path.join(res_dir, "solver_audit.json"),
        ...
        result_contract_failed = not required_result_files_complete
        usable_for_main_analysis = bool(
            usable_for_main_analysis and required_result_files_complete
        )
        hard_failed = bool(hard_failed or result_contract_failed)
        requires_manual_review = bool(
            requires_manual_review or not usable_for_main_analysis
        )
        result_contract_status = (
            "PASS_STRICT" if required_result_files_complete else "FAIL_STRUCTURAL"
        )

        rt_only_audit["result_contract_status"] = result_contract_status
        rt_only_audit["required_result_files_complete"] = (
            required_result_files_complete
        )
        rt_only_audit["required_result_files_missing"] = list(
            required_result_files_missing
        )
        rt_only_audit["solver_strict_for_main_analysis"] = bool(
            solver_strict_for_main_analysis
        )
        rt_only_audit["usable_for_main_analysis"] = bool(
            usable_for_main_analysis
        )
        rt_only_audit["requires_manual_review"] = bool(requires_manual_review)
        rt_only_audit["hard_failed"] = bool(hard_failed)
        ai_execution_audit["result_contract_status"] = result_contract_status
        ai_execution_audit["required_result_files_complete"] = (
            required_result_files_complete
        )
        ai_execution_audit["required_result_files_missing"] = list(
            required_result_files_missing
        )
        ai_execution_audit["usable_for_main_analysis"] = bool(
            usable_for_main_analysis
        )
        ai_execution_audit["requires_manual_review"] = bool(
            requires_manual_review
        )
        ai_execution_audit["hard_failed"] = bool(hard_failed)
        ai_execution_audit["usable_for_main_analysis_rule"] = (
            "RT-only main-analysis use requires an OPTIMAL solver status, a "
            "non-structural AI/RT-only audit, and a complete required-file contract."
        )
        ai_interface_metadata["rt_only_audit"] = rt_only_audit
        ai_interface_metadata["ai_execution_audit"] = ai_execution_audit
        ai_interface_metadata["result_contract"] = {
            "status": result_contract_status,
            "required_result_files_complete": required_result_files_complete,
            "required_result_files_missing": list(required_result_files_missing),
            "usable_for_main_analysis": bool(usable_for_main_analysis),
            "requires_manual_review": bool(requires_manual_review),
            "hard_failed": bool(hard_failed),
        }
        audit_summary_row.update(
            {
        ...
        result_dict["rt_only_audit"] = rt_only_audit
        result_dict["usable_for_main_analysis"] = bool(usable_for_main_analysis)
        result_dict["requires_manual_review"] = bool(requires_manual_review)
        result_dict["hard_failed"] = bool(hard_failed)

        # The result contract is known only after every artifact has been
        # written. Rewrite all status surfaces so they cannot disagree.
        with open(
            os.path.join(res_dir, "rt_only_audit.json"),
            "w",
            encoding="utf-8",
        ...
        run_status = {
            ...
            "network_tier_policy_enabled": bool(use_network_tier_caps),
            "planning_network_tier_policy_binding": False,
            "planning_hosting_share_policy_binding": False,
            ...
            "rt_only_audit_written": os.path.exists(
                os.path.join(res_dir, "rt_only_audit.json")
            ),
            ...
            "note": (
                "The dedicated RT-only variant writes diagnostics before raising "
                "on any structural audit or required-file contract failure."
            ),
            "usable_for_main_analysis_rule": (
                "RT-only main-analysis use requires an OPTIMAL solver status, a "
                "non-structural AI/RT-only audit, and a complete required-file contract."
        ...
        if strict_postsolve_ai_audit and use_external_ai_load and hard_failed:
            msg = (
                "RT-only structural or result-contract failure after postprocess: "
                f"overall_ai_audit_status={overall_ai_audit_status}; "
                f"rt_only_audit_status={rt_only_audit_status}; "
                f"result_contract_status={result_contract_status}; "
                f"ai_energy_audit_status={ai_energy_audit_status}; "
                f"cluster_od_allocation_audit_status={cluster_od_allocation_audit_status}; "
                f"destination_power_cap_audit_status={destination_power_cap_audit_status}; "
                f"s4_od_no_advance_audit_status="
                ...
                "See rt_only_audit.json, ai_execution_audit.json and "
                "run_status.json for details."
            )
        ...
        fail_status = {
            ...
            "runtime_enabled": True,
            "planning_anchor": RT_ONLY_PLANNING_ANCHOR,
            "solver_status": _status_name_map.get(model.status, "OTHER"),
            "solver_terminal_status": int(model.status),
            "sol_count": int(model.SolCount),
            ...
            "usable_for_main_analysis": False,
        ...
        raise RuntimeError(
            "RT-only solver terminated without a usable feasible solution: "
            f"status={int(model.status)}, SolCount={model.SolCount}."
        )
